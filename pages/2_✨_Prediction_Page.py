...
st.sidebar.success("Water Quality Predictor")
...
def show_predict_page1():
    ...
    if ok:
        ...
        if quality == 0:
            st.subheader(
                f"The predicted quality is VERY POOR! You are strongly adviced not to consume it")
        ...
        if quality > 1 and quality <= 1.5:
            st.subheader(
                f"The predicted quality is Good and you can drink it by simply boiling it")
        if quality > 1.5:
            st.subheader(
                f"The predicted quality is Excellent and you can drink it directly")
